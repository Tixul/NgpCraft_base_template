...
### `apply_live_zip.py`
**Applique un ZIP exporté du NgpCraft Live Editor sur ce template, et patche
le makefile pour que les sources utilisateur supplémentaires soient compilées.**

Workflow typique : le joueur prototype son jeu dans l'app NgpCraft Live Editor
(navigateur ou Android), clique "Export ZIP", récupère le `.zip` sur sa machine
de build, puis lance ce script pour intégrer ses sources et générer la ROM.

#### Comportement détaillé

1. **Validation du ZIP.** Liste les entrées, refuse tout chemin qui n'est pas
   `src/<name>.[ch]` (regex `^src/[A-Za-z_][\w-]*\.[ch]$`). Toute entrée
   contenant `..`, un chemin absolu, ou un backslash est rejetée — aucune
   extraction hors de `src/`.
2. **Extraction.** Chaque entrée valide est écrite à son chemin (par défaut
   `<projet>/src/<name>.c|h`), avec écrasement silencieux des fichiers
   existants. Le dossier `src/` est créé si absent.
3. **Patch makefile.** Le script scanne ensuite `<projet>/src/*.c`
   (top-level uniquement, jamais les sous-dossiers `src/core/`, `src/gfx/`,
   etc.), exclut `main.c` (déjà couvert par la liste OBJS de base), et
   regénère un bloc délimité entre marqueurs :
   ...
   - **Premier run** : si le bloc n'existe pas encore, il est injecté juste
     après la dernière ligne de la liste OBJS de base, repérée via la ligne
     d'ancrage `$(OBJ_DIR)/src/core/ngpc_syspatch.rel` (si l'ancre est
     introuvable — version modifiée du makefile — le script abort proprement
     avec un message explicite).
   - **Runs suivants** : le bloc existant est remplacé in-place. Idempotent :
     si rien n'a changé, le makefile n'est même pas réécrit (`makefile
     already up to date — no changes.`).
   - Si aucune source utilisateur n'existe au-delà de `main.c`, le bloc est
     quand même présent avec un commentaire `# (no extra user sources)` pour
     que le re-patch fonctionne plus tard.

4. **Backup.** Avant toute écriture, l'ancien makefile est copié dans
   `makefile.bak` (uniquement si un patch est effectivement appliqué).

#### Arguments

| Argument | Type | Défaut | Rôle |
|---|---|---|---|
| `zip` (positionnel, requis) | chemin | — | ZIP exporté depuis le live editor (`ngpcraft-project-*.zip`). |
| `--project-root` | chemin | parent de `tools/` (auto-détecté) | Racine du projet à patcher. Override pour cibler un checkout différent du template courant. |
| `--dry-run` | flag | off | Imprime le plan complet (entrées ZIP, fichiers à écraser/créer, sources détectées, état du makefile) sans rien écrire sur disque. |

#### Exemples

```sh
# Cas standard : depuis la racine du base template
py -3 tools/apply_live_zip.py path/to/ngpcraft-project-2026-05-17.zip

# Preview sans toucher au disque
py -3 tools/apply_live_zip.py path/to/zip --dry-run

# Cibler un autre checkout (utile si tools/ est partagé entre projets)
py -3 tools/apply_live_zip.py path/to/zip --project-root C:/dev/MonJeu
...
- Seules les sources à la racine `src/*.c` sont auto-ajoutées au makefile.
  Les sources hand-written dans `src/<subdir>/` (par ex. `src/gameplay/`)
  restent à ajouter à la liste OBJS manuellement.
- Le bloc marqueur est en `OBJS += …` et ne peut donc pas être inséré au
  milieu d'un `OBJS = \` continué — c'est pour ça qu'il vient toujours après
  l'ancre.
